...
export default async function handler(
  ...
  if (req.method == "GET") {
    if (req.query.products && req.query.products.length < 2) {
      const { status, data } = await GetDataDetail(
        "products",
        req.query.products[0]
      );
      if (status) {
        res.status(200).json({
          status,
        ...
        res.status(401).json({
          ...
          statusCode: 401,
          message:
            "Data products with id " + req.query.products[0] + " not found",
          data: null,
        });
      }
    } else {
    ...
    const { status, data }:any = await PostData("products", req.body);
    if (status) {
      res.status(200).json({
        status,
        statusCode: 200,
    ...
    res.status(401).json({
      status,
      statusCode: 401,
      message: "failed to create data products",
      data: false,
    });
